...
cd "`dirname $0`"
NAME="`grep ^Name golang-x-text.spec |cut -d: -f2- |xargs echo`"
VERSION="`grep ^Version golang-x-text.spec |cut -d: -f2- |xargs echo`"
URL="`grep ^URL golang-x-text.spec |cut -d: -f2- |xargs echo`"
SOURCE="`grep ^Source0 golang-x-text.spec |cut -d: -f2- |sed -e "s,%{name},$NAME,g; s,%{version},$VERSION,g;" |xargs echo`"
